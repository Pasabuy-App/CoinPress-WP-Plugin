<?php
	...
	*/
	...
	function hp_dbhook_activate(){
		...
		if($wpdb->get_var( "SHOW TABLES LIKE '$tbl_wallet'" ) != $tbl_wallet) {
			$sql = "CREATE TABLE `".$tbl_wallet."` (";
				...
				$sql .= " `date_created` datetime DEFAULT NULL COMMENT 'Date wallet was created', ";
				$sql .= "PRIMARY KEY (`ID`), ";
		...
		if($wpdb->get_var( "SHOW TABLES LIKE '$tbl_currencies'" ) != $tbl_currencies) {
			$sql = "CREATE TABLE `".$tbl_currencies."` (";
				...
				$sql .= " `date_created` datetime DEFAULT NULL COMMENT 'Date of currency creation', ";
				$sql .= "PRIMARY KEY (`ID`) ";
	...
	add_action( 'activated_plugin', 'hp_dbhook_activate');